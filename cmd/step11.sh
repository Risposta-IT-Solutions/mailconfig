...
echo "Inserting new email into the database..." > /home/logs/step11.log
...
if [ $? -eq 0 ]; then
  echo "Email '$PREFIX@$DOMAIN' inserted into the database successfully." >> /home/logs/step11.log
else
  echo "Failed to insert email into the database." >> /home/logs/step11.log
  exit 1
fi

# Create the directory for the new email
echo "Creating directory for email $PREFIX@$DOMAIN..." >> /home/logs/step11.log
sudo mkdir -p /var/mail/vhosts/$DOMAIN/$PREFIX

# Make directories for Dovecot mail storage
echo "Creating Dovecot mail directories..." >> /home/logs/step11.log
sudo maildirmake.dovecot /var/mail/vhosts/$DOMAIN/$PREFIX

# Change ownership of the email directories
echo "Changing ownership of mail directories..." >> /home/logs/step11.log
sudo chown -R vmail:vmail /var/mail/vhosts/$DOMAIN/$PREFIX

# Confirm the operations were successful
if [ $? -eq 0 ]; then
  echo "Directory and permissions set up successfully for $PREFIX@$DOMAIN." >> /home/logs/step11.log
else
  echo "Failed to set up directory or permissions for $PREFIX@$DOMAIN." >> /home/logs/step11.log
  exit 1
fi
